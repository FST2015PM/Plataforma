...
public class AppServices {
    ...
    public Response aggregateData(@PathParam("dsName") String dataSourceId, @Context UriInfo context) {
        ...
        if (hasAccess(httpRequest)) {
            ...
            String ffield = params.getFirst("filterField");
            String fvalue = params.getFirst("filterValue");
            String group = params.getFirst("groupBy");
            String oper = params.getFirst("operation");
            String operVal = params.getFirst("displayField");
            String sort = params.getFirst("sort");

            if (null == operVal || null == oper || operVal.isEmpty() || oper.isEmpty()) {
                return Response.status(Status.BAD_REQUEST).build();
            }

            DataList<DataObject> dl = FSTUtils.API.buildAggregationPipeline(ffield, fvalue, group, operVal, oper, sort);
            //System.out.println(dl);
            DataObject q = new DataObject();
            q.put("data", dl);

            try {
                DataObject ret = ds.aggregate(q, true);
