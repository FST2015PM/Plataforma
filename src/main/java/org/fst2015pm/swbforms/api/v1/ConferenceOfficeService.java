...
public class ConferenceOfficeService {
    ...
    public ConferenceOfficeService() {
    ...
    public Response getOffices(@Context UriInfo context) {
        ...
        SWBDataSource ds = engine.getDataSource("ConferenceOffice");
    ...
    @Path("/{objId}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getConferenceOffice(@PathParam("objId") String oId) {
        HttpSession session = httpRequest.getSession();
        SWBScriptEngine engine = DataMgr.initPlatform("/WEB-INF/dbdatasources.js", session);
    ...
    public Response addConferenceOffice(String content) throws IOException {
        HttpSession session = httpRequest.getSession();
        SWBScriptEngine engine = DataMgr.initPlatform("/WEB-INF/dbdatasources.js", session);
        SWBDataSource ds = engine.getDataSource("ConferenceOffice");
        ...
        if (null == ds) return Response.status(500).build();
        ...
        try {
            ...
            while(it.hasNext()) {
                ...
                if (null != response && 0 == response.getInt("status")) {
                    ...
                    if (null != imgName && null != imgContent) {
                        ...
                        String path = context.getRealPath("/") + "public/images/ConferenceOffice/" + oId;
                        if (FSTUtils.FILE.storeBase64File(path, imgName, imgContent)) {
                            ...
                            dlist.put("image", requestUrl + "/public/images/ConferenceOffice/" + oId + "/" + imgName);
                            ds.updateObj(dlist);
                        }
                    }
            ...
            logger.logActivity(usr.getString("fullname"), usr.getId(), true, "ADD", "Oficinas de congresos");
